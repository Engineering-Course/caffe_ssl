...
if is_server
  ...
  elseif strcmp(dataset, 'human')
    VOC_root_folder = '/home/gongke/human_dataset/';
else
    error('Wrong dataset');
  end
else
...
output_mat_folder = fullfile('/home/gongke/deeplab_v2', dataset, feature_name, model_name, testset, feature_type);

save_root_folder = fullfile('/home/gongke/deeplab_v2', dataset, 'res', feature_name, model_name, testset, feature_type, post_folder);
...
elseif strcmp(dataset, 'human')
  seg_res_dir = [save_root_folder '/results/human/'];
...
if is_mat
  ...
  for i = 1 : numel(output_dir)
    ...
    elseif strcmp(dataset, 'human')
      img = imread(fullfile(VOC_root_folder, 'dataset/images', [img_fn, '.jpg']));
    end
    
    img_row = size(img, 1);
    img_col = size(img, 2);
    %size(raw_result)
    %fprintf('img_row: %d, img_col: %d \n', img_row, img_col);
    result = raw_result(1:min(513,img_row), 1:min(513,img_col), :);
